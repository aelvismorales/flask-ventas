...
def login():
    ...
    if usuario is not None:
        if usuario.verificar_contraseña(u_contraseña.strip()):
            ...
            return jsonify({"mensaje":"Inicio de sesion correcto","http_code": 200,'token':token,'rol':usuario.get_rol(),"nombre_usuario":usuario.get_nombre_usuario(),"id":usuario.get_id()}),200
        else:
...
def token_still_valid(current_user):
    ...
    return jsonify({"mensaje":"Token still valid","http_code": 200,'rol':current_user.get_rol(),'token':token,"nombre_usuario":current_user.get_nombre_usuario(),"id":current_user.get_id()}),200
